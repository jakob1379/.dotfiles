...
*** Ansible
    ...
    (use-package ansible
      :ensure t
      :bind (("C-c b" . 'ansible-decrypt-buffer)
             ("C-c g" . 'ansible-encrypt-buffer))
      :config (setq ansible-vault-password-file "~/.become_passwords.yml")
      )
    #+END_SRC
...
*** Flyspell Spellchecking
    ...
    #+END_SRC
*** Gitlab
    Different packages for gitlab integration and tools
**** Gitlab CI/CD mode
     highlighting and syntax checking in gitlab CI/CD
     #+BEGIN_SRC emacs-lisp
...
**** Code Formatting
     ...
     (use-package python-black
       :ensure t
       :mode ("\\.py\\'" . python-mode)
       :bind (("C-c b b" . 'elpy-black-fix-code)
